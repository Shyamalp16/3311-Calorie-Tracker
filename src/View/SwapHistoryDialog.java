...
import models.Food;
import controller.SwapHistoryController;
import javax.swing.*;
...
public class SwapHistoryDialog extends JDialog {
    ...
    private SwapHistoryController swapHistoryController;
    private int userId;
    ...
    public SwapHistoryDialog(JFrame parent, int userId) {
        ...
        this.userId = userId;
        this.swapHistoryController = new SwapHistoryController();
        initUI();
        loadSwapHistory();
    }
    ...
    private void loadSwapHistory() {
        ...
        List<SwapHistory> swapHistories = swapHistoryController.getSwapHistory(userId);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        
        for (SwapHistory swap : swapHistories) {
            Food originalFood = swapHistoryController.getFoodById(swap.getOriginalFoodId());
            Food swappedFood = swapHistoryController.getFoodById(swap.getSwappedFoodId());
    ...
    private void revertSwap(ActionEvent e) {
        ...
        if (result == JOptionPane.YES_OPTION) {
            try {
                boolean success = swapHistoryController.revertSwap(selectedSwap.getHistoryId(), userId);
                ...
                if (success) {
                    ...
                    if (getParent() instanceof View.Dashboard) {
                        ((View.Dashboard) getParent()).refreshAllTabs();
                    }
                } else {
                    JOptionPane.showMessageDialog(this, 
